...
resource "aws_api_gateway_rest_api" "default" {
  ...
  dynamic "endpoint_configuration" {
    ...
    content {
      ...
      vpc_endpoint_ids = endpoint_configuration.value["type"] == ["PRIVATE"] ? [endpoint_configuration.value["vpc_endpoint_ids"]] : null
    }
  }
  tags = var.tags
}
    ...
    "Statement": [
        {
            ...
            "Resource": "arn:aws:execute-api:${var.region}:${var.AccountId}:${aws_api_gateway_rest_api.default[count.index].id}/*",
            "Condition": {
                "StringNotEquals": {
...
resource "aws_api_gateway_resource" "default" {
  count       = length(var.path_parts) > 0 && var.enabled ? length(var.path_parts) : 0
...
resource "aws_api_gateway_model" "default" {
  count        = var.model_count > 0 && var.enabled ? var.model_count : 0
  rest_api_id  = aws_api_gateway_rest_api.default.*.id[0]
...
resource "aws_api_gateway_integration" "default" {
  count                   = length(var.uri) > 0 && var.enabled ? length(var.uri) : 0
  rest_api_id             = aws_api_gateway_rest_api.default.*.id[0]
  resource_id             = aws_api_gateway_resource.default.*.id[count.index]
  ...
  connection_id           = aws_api_gateway_vpc_link.default[count.index].id
...
resource "aws_api_gateway_method_response" "default" {
  count               = length(aws_api_gateway_method.default.*.id) > 0 && var.enabled ? length(aws_api_gateway_method.default.*.id) : 0
  rest_api_id         = aws_api_gateway_rest_api.default.*.id[0]
  resource_id         = aws_api_gateway_resource.default.*.id[count.index]
  http_method         = aws_api_gateway_method.default.*.http_method[count.index]
  status_code         = length(var.status_codes) > 0 ? element(var.status_codes, count.index) : ""
...
resource "aws_api_gateway_integration_response" "default" {
  count       = length(aws_api_gateway_integration.default.*.id) > 0 && var.enabled ? length(aws_api_gateway_integration.default.*.id) : 0
  rest_api_id = aws_api_gateway_rest_api.default.*.id[0]
  resource_id = aws_api_gateway_resource.default.*.id[count.index]
  http_method = aws_api_gateway_method.default.*.http_method[count.index]
...
# Define the CORS-related resources using dynamic blocks
resource "aws_api_gateway_method" "options_method" {
  count         = var.enable_cors && var.enabled ? length(var.path_parts) : 0
  rest_api_id   = aws_api_gateway_rest_api.default.*.id[0]
  resource_id   = aws_api_gateway_resource.default.*.id[count.index]
  http_method   = "OPTIONS"
  authorization = "NONE"
...
resource "aws_api_gateway_method_response" "options_200" {
  count       = var.enable_cors && var.enabled ? length(aws_api_gateway_method.default.*.id) : 0
  rest_api_id = aws_api_gateway_rest_api.default.*.id[0]
  resource_id = aws_api_gateway_resource.default.*.id[count.index]
...
resource "aws_api_gateway_integration_response" "options_integration_response" {
  count       = var.enable_cors && var.enabled ? length(aws_api_gateway_integration.options_integration.*.id) : 0
  rest_api_id = aws_api_gateway_rest_api.default.*.id[0]
  resource_id = aws_api_gateway_resource.default.*.id[count.index]
  http_method = aws_api_gateway_method.options_method.*.http_method[count.index]
...
resource "aws_api_gateway_deployment" "default" {
  count = var.deployment_enabled && var.enabled ? 1 : 0
...
resource "aws_api_gateway_client_certificate" "default" {
  count       = var.cert_enabled && var.enabled ? 1 : 0
...
resource "aws_api_gateway_stage" "default" {
  count = var.deployment_enabled && var.enabled && var.stage_enabled && length(var.stage_names) > 0 ? length(var.stage_names) : 0
...
resource "aws_api_gateway_stage" "with_log" {
  count = var.deployment_enabled && var.enabled && var.stage_enabled && var.api_log_enabled && length(var.stage_names) > 0 ? length(var.stage_names) : 0
...
resource "aws_api_gateway_stage" "stages" {
  count = var.deployment_enabled && var.enabled && var.stage_enabled && var.enable_cors ? length(var.stage_names) : 0
...
resource "aws_api_gateway_authorizer" "default" {
  count                            = var.authorizer_count > 0 && var.enabled ? var.authorizer_count : 0
  rest_api_id                      = aws_api_gateway_rest_api.default.*.id[0]
...
resource "aws_api_gateway_gateway_response" "default" {
  count         = var.gateway_response_count > 0 && var.enabled ? var.gateway_response_count : 0
  rest_api_id   = aws_api_gateway_rest_api.default.*.id[0]
...
resource "aws_api_gateway_vpc_link" "default" {
  count       = var.vpc_link_count > 0 && var.enabled ? var.vpc_link_count : 0
...
resource "aws_api_gateway_api_key" "default" {
  count       = var.key_count > 0 && var.enabled ? var.key_count : 0
